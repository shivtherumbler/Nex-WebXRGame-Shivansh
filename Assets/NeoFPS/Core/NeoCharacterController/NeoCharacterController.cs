...
namespace NeoCC
{
    ...
    public class NeoCharacterController : MonoBehaviour, INeoCharacterController, INeoCharacterVariableGravity, INeoSerializableComponent
    {
        ...
        public Vector3 up
        {
            ...
            set
            {
                if (!orientUpWithGravity || m_Gravity.sqrMagnitude <= 0.0000001f)
                    SetUpVector(value);
#if UNITY_EDITOR
                else
        ...
        public bool orientUpWithGravity
        {
            ...
            set
            {
                if (!m_OrientUpWithGravity && value && m_Gravity.sqrMagnitude > 0.0000001f)
                    SetUpVector(-m_Gravity.normalized);
        ...
        /// <summary>
        /// The maximum angle from vertical where a surface is classed as a wall.
        /// </summary>
        ...
        {
            // Skip if time is effectively zero
        ...
        void CheckGrounding ()
        {
            ...
            {
                hitGround = Physics.SphereCast(bottom, radius - m_SkinWidth, -m_CurrentUp, out m_Hit, m_GroundingCheckDistance, collisionMask, QueryTriggerInteraction.Ignore);
                if (hitGround)
                {
                    Vector3 localHitNormal = m_InverseRotation * m_Hit.normal;
                    ...
                    hitGround = Physics.Raycast(bottom, -m_CurrentUp, out m_Hit, radius + 0.05f, collisionMask, QueryTriggerInteraction.Ignore); // Hacky workaround for Unity physics bug where sphere casts against edges fail
            }
            ...
            if (hitGround)
            {
                ...
                {
                    if (m_Hit.rigidbody != m_PlatformRigidbody)
                    {
                        m_PlatformRigidbody = m_Hit.rigidbody;
        ...
        void Depenetrate()
        {
            ...
            while (true)
            {
                ...
                for (int i = 0; i < overlaps; ++i)
                {
                    ...
                    // Discard if part of character heirarchy
                    Transform t = m_OverlapColliders[i].attachedRigidbody.transform;
                    if (CheckIgnoreTransform(t))
                        continue;
        ...
        public void Teleport(Vector3 position, Quaternion rotation, bool relativeRotation = true, bool ignoreTilt = false)
        {
            ...
            m_Rigidbody.position = m_LocalTransform.position = m_StartPosition = m_TargetPosition = position;
            m_Rigidbody.interpolation = RigidbodyInterpolation.Interpolate;
