	...
	public class ScenePoolHandler : MonoBehaviour, INeoSerializableComponent
    {
        ...
        class Pool
        {
            ...
            public void Grow()
            {
                ...
                if (prototypeNsgo != null)
                {
                    ...
                    {
                        var obj = NeoPrefabFactory.InstantiateFromID<PooledObject>(prototypeNsgo.prefabStrongID, m_Counter++, poolNsgo, poolTransform);
            ...
            public Pool(PooledObject proto, NeoSerializedGameObject protoNsgo, NeoSerializedGameObject pNsgo, NeoSerializedGameObject aNsgo)
            {
                ...
                for (int i = 1; i < highest; ++i)
                {
                    ...
                    {
                        var obj = NeoPrefabFactory.InstantiateFromID<PooledObject>(prototypeNsgo.prefabStrongID, i, poolNsgo, poolTransform);
                        obj.gameObject.SetActive(false);
                        obj.poolTransform = poolTransform;
                    }
                }
        ...
        public void CreatePool (PooledObject prototype, int total, int startCount)
		{
			...
			else
            {
                ...
                if (m_Nsgo == null || prototypeNsgo == null || !NeoSerializedObjectRegistry.IsPrefabRegistered(prototypeNsgo.prefabStrongID))
                {
                    // Create heirachy
                ...
                else
                {
                    // Create heirachy
                    var nsgo = m_Nsgo;
                    NeoSerializedGameObject poolRoot = nsgo.CreateNewChildObject(prototype.name, prototypeNsgo.prefabStrongID);
                    NeoSerializedGameObject activeNsgo = poolRoot.CreateNewChildObject("Active", 1);
                    NeoSerializedGameObject poolNsgo = poolRoot.CreateNewChildObject("Pool", -1);
        ...
        public void ReadProperties(INeoDeserializer reader, NeoSerializedGameObject nsgo)
        {
            ...
            {
                var prefab = NeoSerializedObjectRegistry.GetPrefab(childObjects[i].serializationKey);
                if (prefab != null)
                {
                    var pooledObject = prefab.GetComponent<PooledObject>();
                    if (pooledObject != null)
                    {
                        var activeNsgo = childObjects[i].nestedObjects[1];
                        var inactiveNsgo = childObjects[i].nestedObjects[-1];
