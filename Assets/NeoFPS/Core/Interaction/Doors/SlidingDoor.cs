...
namespace NeoFPS
{
    ...
    public class SlidingDoor : DoorBase
    {
        ...
        private static readonly NeoSerializationKey k_OriginalPositionsKey = new NeoSerializationKey("positions");
        ...
        public override void WriteProperties(INeoSerializer writer, NeoSerializedGameObject nsgo, SaveMode saveMode)
        {
            ...
            if (m_Initialised)
            {
                var originalPositions = new Vector3[m_Sections.Length];
                for (int i = 0; i < m_Sections.Length; ++i)
                    originalPositions[i] = m_Sections[i].originalPosition;

                writer.WriteValues(k_OriginalPositionsKey, originalPositions);
            }
        }
        ...
        public override void ReadProperties(INeoDeserializer reader, NeoSerializedGameObject nsgo)
        {
            ...
            if (reader.TryReadValues(k_OriginalPositionsKey, out Vector3[] originalPositions, null) && originalPositions != null)
            {
                for (int i = 0; i < originalPositions.Length && i < m_Sections.Length; ++i)
                    m_Sections[i].originalPosition = originalPositions[i];
