...
namespace NeoFPSEditor.SaveGames
{
    ...
    public class SaveGameManagerEditor : NeoSerializedSceneEditorBase
    {
        ...
        readonly GUIContent k_SubFolderLabel = new GUIContent("Sub-Folder", "The texture to use. This should start with an alpha-numeric character");
        readonly GUIContent k_CustomPathLabel = new GUIContent("Custom Path", "The custom path to use. Make sure that it ends with a /");

        public override void OnInspectorGUI()
        {
            var enabledProp = serializedObject.FindProperty("m_EnableSaveSystem");
        ...
        void InspectLocation()
        {
            ...
            if (pathProp.enumValueIndex < 2)
            {
                var input = EditorGUILayout.DelayedTextField(k_SubFolderLabel, subFolderProp.stringValue);
                if (input != subFolderProp.stringValue)
                    subFolderProp.stringValue = SaveGameUtilities.FilterPathString(input);
            }
            else
                EditorGUILayout.PropertyField(subFolderProp, k_CustomPathLabel);

            if (GUILayout.Button("Explore To Folder"))
            {
                var cast = target as SaveGameManager;
                cast.CheckRootSaveFolder();
                Application.OpenURL(cast.GetRootSaveFolder());
            }
        ...
        {
            var useProfilesProp = serializedObject.FindProperty("m_UseProfiles");
            EditorGUILayout.PropertyField(useProfilesProp);
            if (useProfilesProp.boolValue)
                EditorGUILayout.PropertyField(serializedObject.FindProperty("m_DefaultProfile"));
        }
