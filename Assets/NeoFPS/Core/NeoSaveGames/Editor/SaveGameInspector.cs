...
namespace NeoSaveGames
{
    ...
    {
        [MenuItem("Tools/NeoFPS/Save System/Save File Inspector", priority = 20)]
        ...
        void InspectCurrentSave()
        {
            ...
            // Open the save directory in explorer
            if (GUILayout.Button("Explore To Folder"))
            {
                Application.OpenURL(GetSaveFolder());
            }
            ...
            else
            {
                ...
                if (GUILayout.Button("Import search strings", GUILayout.Width(150)))
                {
                    ...
                    /*
                    var full = GUIUtility.systemCopyBuffer;
                    ...
                    foreach(var line in lines)
                    {
                        ...
                        if (start != -1 && end != -1)
                        {
                            var keyName = line.Substring(start + 1, end - start - 1);
        ...
        void AddJsonKeyMaps()
        {
            var guids = AssetDatabase.FindAssets("NeoSaveGameSearchStrings.json");
            if (guids.Length > 0)
            {
                var textFile = AssetDatabase.LoadAssetAtPath<TextAsset>(AssetDatabase.GUIDToAssetPath(guids[0]));
                var import = JsonUtility.FromJson<DictionaryExport>(textFile.text);
                import.AddToDictionary(m_KeyMap);
            }
        }
