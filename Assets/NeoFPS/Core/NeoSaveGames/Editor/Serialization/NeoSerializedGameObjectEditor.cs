...
namespace NeoSaveGames.Serialization
{
    ...
    public class NeoSerializedGameObjectEditor : Editor
    {
        ...
        void GetSaveDetails()
        {
            ...
            var guids = AssetDatabase.FindAssets("t:NeoSaveGames.SaveGameManager");
            ...
            {
                var root = cast.transform.root;
                if (PrefabUtility.IsPartOfAnyPrefab(root))
                    m_RegisterablePrefab = root.GetComponent<NeoSerializedGameObject>();
                else
                    m_RegisterablePrefab = null;
            }
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (cast.gameObject.scene.IsValid() && PrefabStageUtility.GetCurrentPrefabStage() == null)
            {
                ...
                else
                {
                    ...
                    if (cast.CheckWillBeSerialized())
                    {
                        info.Add("- Object WILL be serialized");

                        if (m_RegisterablePrefab == null)
                            info.Add("- Object is not part of a prefab");
                        else
                        {
                ...
                else
                {
                    ...
                    if (parent != null)
                    {
                        key = parent.nestedObjects.GetSerializationKeyForObject(cast);
                    }
                    else
                    {
                        if (cast.transform.parent == null)
                        {
                            if (sceneInfo != null)
                            {
                                key = sceneInfo.nestedObjects.GetSerializationKeyForObject(cast);
                                if (key == 0)
            ...
            EditorGUILayout.PropertyField(serializedObject.FindProperty("m_MoveToLocalScene"));
            ...
            //EditorGUILayout.PropertyField(serializedObject.FindProperty("m_IsPrefabRoot"));
