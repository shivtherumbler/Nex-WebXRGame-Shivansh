...
namespace NeoSaveGames.Serialization
{
    ...
    public static class NeoSerializationUtilities
    {
        ...
        public static bool WriteTransformReference(INeoSerializer writer, Transform value, NeoSerializedGameObject pathFrom, int hash)
        {
            ...
            {
                Debug.LogError("Invalid parameters when writing transform reference");
                return false;
            }

            // Check if transform reference is null
            if (value == null)
                writer.WriteValues(hash, (int[])null);
            else
            {
                // Check if component has an attached NeoSerializedGameObject
                var otherNsgo = value.GetComponent<NeoSerializedGameObject>();
                if (otherNsgo == null)
                {
                    Debug.LogError("Can only write component references for components with a NeoSerializedGameObject behaviour on the same object: " + value.name);
                    return false;
                }
                ...
                if (otherNsgo != pathFrom)
                {
                    ...
                    if (!otherNsgo.CheckWillBeSerialized())
                    {
                ...
                s_ReverseChain.Add(GetPersistentComponentID(value));
        ...
        private static bool WriteObjectReference(INeoSerializer writer, NeoSerializedGameObject value, NeoSerializedGameObject pathFrom, int hash)
        {
            ...
            else
            {
                ...
                if (value != pathFrom)
                {
                    ...
                    if (!value.CheckWillBeSerialized())
                    {
                ...
                default:
                    {
                        ...
                        {
                            // Grab start of chain
                            var next = targetScene.nestedObjects[chain[1]];
                            if (next == null)
                                return false;
                            ...
                            for (int i = 2; i < chain.Length - 1; ++i)
                            {
                                ...
                                next = next.nestedObjects[chain[i]];
                                if (next == null)
                                    return false;
                            }
                            owner = next;
        ...
        public static bool TryReadNeoSerializedGameObjectReference(INeoDeserializer reader, out NeoSerializedGameObject output, NeoSerializedGameObject pathFrom, int hash)
        {
            ...
            if (reader.TryReadValues(hash, out chain, null))
            {
                ...
                else
                {
                    ...
                    {
                        Debug.Log($"Object reference not found. Source object = {pathFrom.gameObject.name}, chain[0] = {chain[0]}");
                        return false;
                    }
                }
            }
        ...
        public static bool TryReadComponentReference<T>(INeoDeserializer reader, out T output, NeoSerializedGameObject pathFrom, int hash) where T : class
        {
            ...
            for(int i = 0; i < s_ComponentBuffer.Count; ++i)
            {
                ...
                //int currentHash = StringToHash(t.ToString()) + count;
            ...
            // Hash wasn't found
            Debug.Log($"Component reference not found. Source object = {pathFrom.gameObject.name}, Component object = {owner.gameObject}", pathFrom.gameObject);
        ...
        public static T GetComponentInParentAllowInactive<T>(Transform t) where T : Component
        {
            while (t != null)
            {
