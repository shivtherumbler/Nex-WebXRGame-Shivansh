...
namespace NeoFPS
{
	...
	public class WeaponAimAmplifier : MonoBehaviour, IAdditiveTransform, INeoSerializableComponent
    {
        ...
        {
            // Ridiculous hack, but aim controller seems to register yaw forward as world forward for 1 frame a couple of frames after start
            // This bypasses the problem until I've worked out why
        ...
        {
            if (reader.TryReadValue(k_RotationKey, out var r, Quaternion.identity) && r.w != 0f)
            {
                rotation = r;
