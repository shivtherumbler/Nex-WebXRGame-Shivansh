...
namespace NeoFPS
{
    public class PoseHandler : IPoseHandler
    {
        ...
        {
            // Reset to zero position, but restart pose lerp
            m_SourcePosition = Vector3.zero;
            m_SourceRotation = Quaternion.identity;
            m_PoseTransform.localPosition = m_SourcePosition;
            m_PoseTransform.localRotation = m_SourceRotation;
        ...
        public void ReadProperties(INeoDeserializer reader)
        {
            ...
            if (reader.TryReadValue(k_NumPosesKey, out numPoses, 0) && numPoses > 0)
            {
                for (int i = 0; i < numPoses; ++i)
                {
                    ...
                    {
                        var existing = GetPose(poseInfo.owner);
                        if (existing != null)
                        {
                            var loaded = poseInfo.poseInformation;
                            existing.position = loaded.position;
                            existing.rotation = loaded.rotation;
                            existing.interpolatePositionOut = loaded.interpolatePositionOut;
                            existing.interpolateRotationOut = loaded.interpolateRotationOut;
                            existing.interpolatePositionIn = loaded.interpolatePositionIn;
                            existing.interpolateRotationIn = loaded.interpolateRotationIn;
                        }
                        else
