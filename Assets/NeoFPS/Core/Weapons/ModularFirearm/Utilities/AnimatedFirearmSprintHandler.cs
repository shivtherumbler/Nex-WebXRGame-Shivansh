...
namespace NeoFPS.ModularFirearms
{
    ...
    [HelpURL("https://docs.neofps.com/manual/weaponsref-mb-animatedfirearmsprinthandler.html")]
    public class AnimatedFirearmSprintHandler : AnimatedSprintAnimationHandler
    {
        [Header("Firearms")]
        ...
        private SprintAimAction m_ActionOnAim = SprintAimAction.StopAnimation;
        ...
        {
            return m_Firearm.animationHandler;
        }
        ...
        private void OnAimStateChanged(IModularFirearm firearm, FirearmAimState state)
        {
            if (m_Aiming)
            {
                if (state == FirearmAimState.HipFire || state == FirearmAimState.ExitingAim)
                {
                    ...
                    {
                        case SprintAimAction.StopAnimation:
                            RemoveAnimationBlocker();
                            break;
                        case SprintAimAction.StopSprinting:
                            RemoveSprintBlocker();
                            break;
                    }
                }
                ...
                case SprintState.NotSprinting:
                    {
                        if (m_Firearm != null)
                        {
                            ...
                            if (m_ActionOnAim == SprintAimAction.CannotAim)
                                m_Firearm.RemoveAimBlocker(this);
                        }
                    }
                    break;
