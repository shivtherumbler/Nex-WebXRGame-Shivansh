...
{
    [HelpURL("https://docs.neofps.com/manual/weaponsref-mb-dotproductobjecttracker.html")]
    public class DotProductObjectTracker : MonoBehaviour, IGuidedProjectileTargetTracker
    {
        ...
        private static Collider[] s_OverlapColliders = new Collider[64];
        ...
        public bool GetTargetPosition(out Vector3 targetPosition)
        {
            ...
            if (m_DetectionTimer <= 0f)
            {
                ...
                for (int i = 0; i < overlaps; ++i)
                {
                    ...
                    // Check distance
                    float dotProduct = Vector3.Dot(Vector3.Normalize(s_OverlapColliders[i].bounds.center - localPosition), localForward);
